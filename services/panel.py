...
class PanelService:
    """Сервис для работы с API 3x-ui панелей"""

    async def add_client(self, panel_host: str, inbound_id: int, client_uuid: str, email: str, settings: str | None = None) -> bool:
        panel = PANELS.get(panel_host)
        if not panel:
            return False
        client_data = json.loads(settings) if settings else {"id": client_uuid, "alterId": 0, "email": email, "limitIp": 0, "totalGB": 0, "expiryTime": 0, "enable": True}
    ...
    async def update_client(self, panel_host: str, inbound_id: int, client_uuid: str, email: str, settings: str | None = None) -> bool:
        panel = PANELS.get(panel_host)
        if not panel:
            return False
        client_data = json.loads(settings) if settings else {"id": client_uuid, "alterId": 0, "email": email, "limitIp": 0, "totalGB": 0, "expiryTime": 0, "enable": True}
    ...
    async def delete_client(self, panel_host: str, inbound_id: int, client_identifier: str) -> bool:
        panel = PANELS.get(panel_host)
        if not panel:
            return False
        del_url = f"{panel['url']}/panel/api/inbounds/{inbound_id}/delClient/{client_identifier}"
        payload = {"id": inbound_id, "settings": json.dumps({"clients": [{"email": client_identifier, "id": client_identifier}]})}
        try:
    ...
    async def inbound_exists(self, panel_host: str, inbound_id: int) -> bool | None:
        panel = PANELS.get(panel_host)
        if not panel:
            return False
